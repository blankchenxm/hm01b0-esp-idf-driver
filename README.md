...
extends that pipeline to Full, QVGA, and monochrome QQVGA modes and replaces
the complete RGB565 frame with two alternating 60-row DMA Strip Buffers.
...
| 6. Multi-mode display | Strip pipeline implemented; hardware validation pending | One mode-driven Full/QVGA/QQVGA flow, automatic crop/destination geometry, two alternating RGB565 DMA Strips, native QQVGA centered with black borders |
...
dropped at frame entry rather than blocking Camera Buffer recycling when a
Strip is still owned by SPI DMA. Accepted frames alternate CPU conversion and
SPI DMA across the two Strip Buffers.
...
| Frame timing | 376 PCK x 533 lines, approximately 30 FPS |
...
The shared internal-DMA workspace is 57,600 bytes. During preflight it stores a
57,600-byte Full/QVGA RAW8 Snapshot or a 19,200-byte QQVGA Snapshot. During
streaming it is split into two 28,800-byte `240x60 RGB565` Strip Buffers. Full
and QVGA submit four alternating Strips per displayed frame; QQVGA submits two
`160x60` Strips and leaves the surrounding black area unchanged. Camera A/B
always follow the uncropped DVP transport. See
[stage6_todo.md](stage6_todo.md) for memory calculations, frame-rate limits,
configuration examples, and the hardware checklist.
